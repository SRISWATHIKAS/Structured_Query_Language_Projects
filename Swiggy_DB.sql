CREATE DATABASE SwiggyDataBase;
...
SELECT CONCAT(FirstName,LastName) AS CUSTOMER_NAME
FROM customers
WHERE CONCAT(FirstName,LastName) LIKE '%Kumar';
...
SELECT * 
FROM delivery
WHERE ISNULL(DeliveryRating);

-- 25. Display deliveries that have received ratings. 
SELECT * 
FROM delivery
WHERE NOT ISNULL(DeliveryRating);
...
SELECT *
FROM payments
WHERE PaymentMethod NOT IN ('Cash','Cash on Delivery') 
GROUP BY PaymentStatus
HAVING PaymentStatus = 'Success';
...
SELECT ROUND((Price+((5/100)*Price)),2) AS 'ADDING_5_%' 
FROM menuitems;
...
SELECT COUNT(PaymentStatus) AS TOTAL_COMPLETED_PAYMENTS
FROM payments
WHERE PaymentStatus = 'Success';
...
SELECT MIN(TotalAmount) AS LOWEST_ORDER_AMOUNT
FROM orders;

-- 49. Display the average customer review rating. 
SELECT AVG(FoodRating) AS AVERAGE_FOOD_RATING
FROM reviews;
SELECT AVG(DeliveryRating) AS AVERAGE_DELIVERY_RATING
FROM reviews;

-- 50. Display the average review rating rounded to two decimal places. 
SELECT ROUND(AVG(FoodRating),2) AS AVERAGE_FOOD_RATING
FROM reviews;
SELECT ROUND(AVG(DeliveryRating),2) AS AVERAGE_DELIVERY_RATING
FROM reviews;
...
SELECT FoodRating,COUNT(FoodRating) AS COUNT
FROM reviews
GROUP BY FoodRating;

SELECT DeliveryRating,COUNT(DeliveryRating) AS COUNT
FROM reviews
...
SELECT PaymentMethod,COUNT(*) AS COUNT,SUM(Amount) AS TOTAL_REVENUE,AVG(Amount) AS AVG_REVENUE,MAX(Amount) AS MAX_REVENUE,MIN(Amount) AS MIN_REVENUE
FROM payments
GROUP BY PaymentMethod;

-- PART C – JOIN Queries (Questions 61–90)
-- 61. Display customer name, order ID, order date, and total amount. 
SELECT CONCAT(FirstName,LastName) AS CUSTOMER_NAME,ORD.OrderID,ORD.OrderDate,ORD.TotalAmount
FROM customers AS C
...
SELECT ORD.OrderID,R.RestaurantName,ORD.OrderDate,ORD.TotalAmount
FROM restaurants AS R
INNER JOIN orders AS ORD
ON R.RestaurantID = ORD.RestaurantID
ORDER BY ORD.OrderID;

-- 63. Display menu item name, category name, and price. 
SELECT MI.ItemName,CategoryName,MI.Price
...
SELECT ORD.OrderID,P.Amount,P.PaymentMethod,P.PaymentStatus
FROM orders AS ORD
...
SELECT CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,R.FoodRating,R.ReviewComment
FROM customers AS C
INNER JOIN reviews AS R
ON C.CustomerID = R.CustomerID;

-- 67. Display restaurant name, review rating, and review comment. 
SELECT RE.RestaurantName,R.FoodRating,R.ReviewComment
...
SELECT CONCAT(C.FirstName,C.LastName),O.OrderID
FROM customers AS C
LEFT JOIN orders AS O
ON C.CustomerID = O.CustomerID;

-- 70. Display all restaurants along with their menu items. 
SELECT R.RestaurantName,MI.ItemName
FROM restaurants AS R
...
SELECT RE.RestaurantName,R.FoodRating
FROM restaurants AS RE
...
SELECT RS.RestaurantName,R.FoodRating
...
SELECT D.DeliveryID,D.OrderID,D.PartnerID,D.AssignedTime,D.PickupTime,D.DeliveryTime,D.DeliveryStatus,D.DeliveryRating,DP.PartnerName
...
SELECT CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,R.RestaurantName,O.TotalAmount,P.PaymentStatus
FROM customers AS C
...
SELECT CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,R.RestaurantName,DP.PartnerName,D.DeliveryStatus
FROM customers AS C
INNER JOIN orders AS O
ON C.CustomerID = O.CustomerID
INNER JOIN restaurants AS R
...
SELECT CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,R.RestaurantName,P.Amount,P.PaymentMethod,RV.FoodRating
FROM customers AS C
INNER JOIN reviews AS RV
ON C.CustomerID = RV.CustomerID
INNER JOIN restaurants AS R
ON RV.RestaurantID = R.RestaurantID
INNER JOIN payments AS P
ON RV.OrderID = P.OrderID;

-- 81. Display each customer's total number of orders. 
SELECT CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,COUNT(O.CustomerID) AS COUNT 
FROM customers AS C
LEFT JOIN orders AS O
ON C.CustomerID = O.CustomerID
...
SELECT R.RestaurantName,COUNT(O.RestaurantID) AS COUNT
FROM restaurants AS R
LEFT JOIN orders AS O
...
SELECT DP.PartnerName,COUNT(D.PartnerID) AS COUNT
FROM deliverypartners AS DP
LEFT JOIN delivery AS D
ON DP.PartnerID = D.PartnerID
...
SELECT CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,R.RestaurantName
FROM customers AS C
...
SELECT R.RestaurantName,R.City,AVG(MI.Price)
FROM restaurants AS R
RIGHT JOIN menuitems AS MI
ON R.RestaurantID = MI.RestaurantID
GROUP BY R.RestaurantName,R.City;

-- 89. Display each food category with the number of menu items. 
SELECT MC.CategoryName,COUNT(MI.CategoryID) AS COUNT
FROM menucategories AS MC
RIGHT JOIN menuitems AS MI
ON MC.CategoryID = MI.CategoryID
...
SELECT CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,R.RestaurantName,P.PaymentStatus,D.DeliveryStatus
FROM customers AS C
...
SELECT NOW() AS 'CURRENT-DATE_AND_CURRENT-TIME';
...
SELECT OrderID,CustomerID,RestaurantID,YEAR(OrderDate) AS ORDER_YEAR,EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 98. Display the order month for every order. 
SELECT OrderID,CustomerID,RestaurantID,MONTH(OrderDate) AS ORDER_MONTH,EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 99. Display the month name for every order. 
SELECT OrderID,CustomerID,RestaurantID,MONTHNAME(OrderDate) AS ORDER_MONTHNAME,EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 100. Display the day of the month for every order. 
SELECT OrderID,CustomerID,RestaurantID,DAY(OrderDate) AS ORDER_DAY,EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 101. Display the weekday name for every order. 
SELECT OrderID,CustomerID,RestaurantID,DAYNAME(OrderDate) AS ORDER_DAY,EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 102. Display the weekday number for every order. 
SELECT OrderID,CustomerID,RestaurantID,WEEKDAY(OrderDate) AS ORDER_DAY,EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 103. Display the week number for every order. 
SELECT OrderID,CustomerID,RestaurantID,WEEKOFYEAR(OrderDate) AS WEEK_NUMBER,EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 104. Display the quarter for every order. 
SELECT OrderID,CustomerID,RestaurantID,QUARTER(OrderDate) AS 'QUARTER',EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 105. Display the day number within the year for every order. 
SELECT OrderID,CustomerID,RestaurantID,DAYOFYEAR(OrderDate) AS DAY_NUM_WITH_YEAR,EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 106. Calculate the number of days between the order date and delivery date. 
SELECT *,DATEDIFF(O.EstimatedDelivery,O.OrderDate) AS DIFFERENCE
FROM orders AS O;

-- 107. Calculate the delivery duration in minutes. 
SELECT *,TIMEDIFF(O.EstimatedDelivery,O.OrderDate) AS DIFFERENCE
FROM orders AS O;

-- 108. Display the expected delivery date by adding two days to the order date. 
SELECT *,DATE_ADD(OrderDate, INTERVAL 2 DAY)AS EXPECTED_DELIVERY_DATE
FROM orders;

-- 109. Display a reminder date three days before the order date. 
SELECT *,DATE_SUB(OrderDate, INTERVAL 3 DAY)AS ORDER_DATE
FROM orders;

-- 110. Add seven days to each order date. 
SELECT *,DATE_ADD(OrderDate, INTERVAL 7 DAY)AS DELIVERY_DATE
FROM orders;

-- 111. Subtract five days from each order date. 
SELECT *,DATE_SUB(OrderDate, INTERVAL 5 DAY)AS DELIVERY_DATE
FROM orders;

-- 112. Display all orders placed during the last thirty days. 
SELECT * 
FROM orders
WHERE OrderDate > DATE_SUB((SELECT MAX(OrderDate) FROM orders),INTERVAL 30 DAY);

-- 113. Display the order date in DD-MM-YYYY format.
SELECT OrderID,CustomerID,RestaurantID,DATE_FORMAT(OrderDate,'%d-%m-%Y'),EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 114. Display the order month and year in "Month YYYY" format. 
SELECT OrderID,CustomerID,RestaurantID,DATE_FORMAT(OrderDate,'%M-%Y'),EstimatedDelivery,OrderStatus,DeliveryAddress,TotalAmount
FROM orders;

-- 115. Display monthly revenue generated from completed payments. 
SELECT MONTHNAME(O.OrderDate) AS 'MONTH',SUM(P.Amount) AS TOTAL_REVENUE
FROM orders AS O
RIGHT JOIN payments AS P
ON O.OrderID = P.OrderID
GROUP BY P.PaymentStatus,MONTHNAME(O.OrderDate)
HAVING P.PaymentStatus = 'Success';

-- 116. Display the daily order count. 
SELECT DATE(OrderDate) AS ORDER_DATE,COUNT(*) AS COUNT
FROM orders
GROUP BY DATE(OrderDate);

-- 117. Display the total number of orders placed each month. 
SELECT MONTHNAME(OrderDate) AS 'MONTHNAME',COUNT(*) AS COUNT
FROM orders
GROUP BY MONTHNAME(OrderDate);

-- 118. Display the total number of orders placed on each weekday. 
SELECT WEEKDAY(OrderDate) AS WEEKDAY,COUNT(*) AS COUNT 
FROM orders
GROUP BY WEEKDAY(OrderDate);

-- 119. Display the average delivery time in minutes. 
SELECT ROUND((TIME_TO_SEC((AVG(TIME(DeliveryTime)))))/60) AS AVERAGE_TIME FROM delivery;

-- 120. Prepare a monthly business summary showing total orders, revenue, and average order value. 
SELECT COUNT(*) AS TOTAL_COUNT,SUM(TotalAmount) AS TOTAL_REVENUE,AVG(TotalAmount) AS AVERAGE_VALUE 
FROM orders;

-- 121. Display all customers with a row number based on their total spending. 
SELECT ROW_NUMBER() OVER(ORDER BY SUM(P.Amount) DESC) AS ROW_RANK,C.CustomerID,CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,SUM(P.Amount) AS TOTAL_SPENDING
FROM customers AS C
INNER JOIN orders AS O
ON C.CustomerID =  O.CustomerID
INNER JOIN payments AS P
...
RANK() OVER(
ORDER BY SUM(O.TotalAmount) DESC) AS REVENUE_RANK
FROM restaurants AS R
INNER JOIN orders AS O
ON R.RestaurantID = O.RestaurantID
GROUP BY R.RestaurantName;
...
SELECT CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,SUM(P.Amount) AS TOTAL_SPENDING,
DENSE_RANK() OVER(
ORDER BY SUM(P.Amount) DESC) AS SPENDING_RANK
FROM customers AS C
INNER JOIN orders AS O
ON C.CustomerID =  O.CustomerID
INNER JOIN payments AS P
...
SELECT CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,SUM(P.Amount) AS TOTAL_SPENDING,
NTILE(4) OVER(
ORDER BY SUM(P.Amount) DESC) AS GROUP_RANK
FROM customers AS C
INNER JOIN orders AS O
ON C.CustomerID =  O.CustomerID
INNER JOIN payments AS P
...
SELECT PaymentID,PaymentDate,PaymentMethod,PaymentStatus,Amount,
LAG(Amount) OVER (ORDER BY PaymentDate) AS PREVIOUS_PAYMENT
FROM payments;

-- 126. Display each payment along with the next payment amount. 
SELECT PaymentID,PaymentDate,PaymentMethod,PaymentStatus,Amount,
LEAD(Amount) OVER (ORDER BY PaymentDate) AS NEXT_PAYMENT
FROM payments;
...
SELECT PaymentID,PaymentDate,Amount,
AVG(Amount) OVER(ORDER BY PaymentDate) AS MOVING_AVERAGE
FROM payments
WHERE PaymentStatus = 'Success';

-- 129. Using a Common Table Expression (CTE), display restaurants with revenue greater than ₹10,000. 
WITH REVENUE AS(
SELECT RestaurantID,SUM(TotalAmount) AS TOTAL_AMOUNT
FROM orders
...
FROM restaurants)
SELECT RE.RestaurantID,R.RestaurantName,RE.TOTAL_AMOUNT
FROM REVENUE AS RE
INNER JOIN REST AS R
ON RE.RestaurantID = R.RestaurantID;
...
FROM orders),
PAY AS( SELECT OrderID,SUM(Amount) AS TOTAL_AMOUNT
FROM payments GROUP BY OrderID)
SELECT C.CustomerID,C.CUSTOMER_NAME,SUM(P.TOTAL_AMOUNT) AS TOTAL_AMOUNT
FROM CUST AS C
INNER JOIN ORD AS O
ON C.CustomerID = O.CustomerID
INNER JOIN PAY AS P
...
SELECT C.CustomerID,CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,SUM(P.Amount)
FROM customers AS C
INNER JOIN orders AS O
ON C.CustomerID = O.CustomerID
INNER JOIN payments AS P
...
WITH ORD AS (
SELECT O.OrderID,O.TotalAmount,R.RestaurantID,R.RestaurantName,AVG(O.TotalAmount) OVER(PARTITION BY R.RestaurantID) AS AVG_AMOUNT
FROM orders AS O
INNER JOIN restaurants AS R ON O.RestaurantID = R.RestaurantID
)
SELECT OrderID, RestaurantName, TotalAmount
FROM ORD
WHERE TotalAmount > AVG_AMOUNT;
...
SELECT DISTINCT PaymentStatus,COUNT(PaymentStatus) OVER(PARTITION BY PaymentStatus) AS COUNT
FROM payments;

-- 135. Display customers who have placed at least one order. 
WITH CUST AS(
SELECT DISTINCT CustomerID,COUNT(CustomerID) OVER(PARTITION BY CustomerID) AS CUST_COUNT
FROM orders)
SELECT CustomerID,CUST_COUNT
FROM CUST
WHERE CUST_COUNT >= 1;
...
FROM restaurants),
ORD AS (
SELECT DISTINCT RestaurantID,SUM(TotalAmount) OVER(PARTITION BY RestaurantID) AS TOTAL_REVENUE
FROM orders)
SELECT R.RestaurantID,R.RestaurantName,O.TOTAL_REVENUE
FROM REVENUE AS R
INNER JOIN ORD AS O
ON O.RestaurantID = R.RestaurantID
...
SELECT C.CustomerID,CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,SUM(O.TotalAmount) AS TOTAL_SPENDING
FROM customers AS C
INNER JOIN orders AS O
ON C.CustomerID = O.CustomerID
GROUP BY C.CustomerID
...
SELECT EXTRACT(HOUR FROM OrderDate) AS ORD_HOUR,COUNT(OrderID) FROM orders
GROUP BY EXTRACT(HOUR FROM OrderDate)
ORDER BY EXTRACT(HOUR FROM OrderDate);

-- 143. Display the average delivery time for each delivery partner. 
SELECT DP.PartnerID,DP.PartnerName,SEC_TO_TIME(AVG(TIME_TO_SEC(TIMEDIFF(D.DeliveryTime,D.PickupTime)))) AS AVG_TIME 
FROM delivery AS D
INNER JOIN deliverypartners AS DP
ON D.PartnerID = DP.PartnerID
...
RANK() OVER(ORDER BY COUNT(PaymentMethod) DESC) AS 'RANK'
FROM payments
WHERE PaymentStatus =  'Success'
GROUP BY PaymentMethod;

-- 146. Display each restaurant's revenue along with its percentage contribution to total revenue. 
SELECT DISTINCT r.RestaurantID,r.RestaurantName,SUM(o.TotalAmount) AS Total_Revenue,
...
SELECT C.CustomerID,CONCAT(C.FirstName,C.LastName) AS CUSTOMER_NAME,COUNT(O.OrderID) AS ORD_COUNT
FROM orders AS O
...
SELECT CustomerID,COUNT(DISTINCT RestaurantID) AS 'count'
FROM orders
GROUP BY CustomerID
...
COUNT(DISTINCT o.RestaurantID) AS Total_restaurants,
(SELECT count(OrderID) FROM orders) AS Total_orders,
(SELECT count(*) FROM payments where PaymentStatus="Success") AS Total_completed_payments,
...
SUM(o.TotalAmount) AS Total_Revenue,AVG(r.Rating) AS AVG_Rating,
AVG(timestampdiff(Minute,d.pickupTime,d.DeliveryTime)) AS AVG_DeliveryTime
FROM orders o
